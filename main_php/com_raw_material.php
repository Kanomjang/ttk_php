...
if ($_POST["add_com_raw_material"]) {
  ...
  add_com_raw_material($id_crmd, $min_stock, $id_emp_use);
...
if ($_POST['save_com_raw_material']) {
  ...
  $id_crmd = $_POST['id_crmd'];
  $min_stock = $_POST['min_stock'];
  save_com_raw_material($id_crm, $id_crmd, $min_stock, $id_emp_use);
}

if ($_GET['delete_com_raw_material']) {
  $id_crm = $_GET['delete_com_raw_material'];
  $id_crm = base64_decode(base64_decode("$id_crm"));
  delete_com_raw_material($id_crm);
...
#################################################
function save_com_raw_material($id_crm, $id_crmd, $min_stock, $id_emp_use)
{
  global $host, $user, $passwd, $database;
  $connect = mysqli_connect($host, $user, $passwd, $database) or die("not connect database");
  $sql_query = "update com_raw_material set id_crmd='$id_crmd', min_stock='$min_stock',id_use='$id_emp_use' where id_crm='$id_crm'";
  mysqli_query($connect, $sql_query);
  mysqli_close($connect);
}
#################################################
function add_com_raw_material( $id_crmd, $min_stock, $id_emp_use)
{
  global $host, $user, $passwd, $database, $id_emp_use;
  $connect = mysqli_connect($host, $user, $passwd, $database) or die("not connect database");
  $sql_query = "insert into com_raw_material(id_crmd,min_stock,id_use) values('$id_crmd','$min_stock','$id_emp_use')";
  mysqli_query($connect, $sql_query);
  mysqli_close($connect);
}
#################################################
function delete_com_raw_material($id_crm)
{
  global $host, $user, $passwd, $database;
  $connect = mysqli_connect($host, $user, $passwd, $database) or die("not connect database");
  $sql_query = "DELETE FROM com_raw_material WHERE id_crm=$id_crm";
  mysqli_query($connect, $sql_query);
  mysqli_close($connect);
}
#################################################
  ...
  while ($row = mysqli_fetch_array($shows)) {
    ...
    $print[$a] = "
		<TR>
            ...
            <TD>&nbsp;&nbsp;$min_stock</TD>
            <TD><A href='com_add_raw_material.php?edit_com_raw_material=$encode_id_crm'  class=\"btn btn-warning\" role=\"button\">แก้ไข</A></TD>
			<TD><A href='#' onclick=\"ShowDelete('com_add_raw_material.php?delete_com_raw_material=$encode_id_crm')\" class=\"btn btn-danger\" role=\"button\">ลบ</A></TD>
		</TR>";
    $a++;
  }
  mysqli_close($connect);
  ...
  <div class=\"content-wrapper\">
    ...
    <section class=\"content-header\">
      ...
      <ol class=\"breadcrumb\">
        ...
        <li class=\"active\">รายละเอียดคลังวัตถุดิบ</li>
      </ol>
    </section>
    <!-- Info boxes -->
    <div class=\"row\">
        ...
        <div class=\"col-xs-12\">
          <div class=\"box\">
            <div class=\"box-header\">
              <h3 class=\"box-title\">รายละเอียดคลังวัตถุดิบ &nbsp;  &nbsp;   $text_ledger</h3>
              <div class=\"box-tools pull-right\">
                ...
                <div class=\"btn-group\">
                  ...
                  <ul class=\"dropdown-menu\" role=\"menu\">
                    <li><a href=\"com_add_raw_material.php?com_add_raw_material=true\"><i class=\"fa fa-plus\"></i> เพิ่มรายละเอียดคลังวัตถุดิบ</a></li>
                    <li> <a href=\"com_raw_materials.php\"><i class=\"fa fa-reply-all\"></i> คลังวัตถุดิบ </a></li>
                  </ul>
                </div>
              </div>
