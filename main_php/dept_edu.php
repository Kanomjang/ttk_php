...
if ($_POST["add_dept_edu"]) {
  ...
  header("location: http://localhost/main_php/dept_edu.php");
  exit(0);
}
if ($_POST['save_dept']) {
...
#################################################
function count_dept_edu($id_dept)
{
  global $host, $user, $passwd, $database;
  $count_dept_edu = 0;
  $connect = mysqli_connect($host, $user, $passwd, $database) or die("not connect database");
  //mysqli_select_db($database,$connect) or die(" ไม่สามารถเลือก database ได้");
  $sql_query = "select count(id_dept) from dept_edu where id_dept='$id_dept'";
  $shows = mysqli_query($connect, $sql_query);
  while ($row = mysqli_fetch_array($shows)) {
...
function select_dept_edu()
{
  ...
  while ($row = mysqli_fetch_array($shows)) {
    ...
    #$count_dept_edu = count_dept_edu($id_dept);
	$count_dept_edu =0;
    $print[$a] = "
		<tr>
			<TD>$name_dept</TD>
			<TD>$count_dept_edu</TD>
			<TD><A href='select_dept_edu.php?select_dept_edu=$encode_id_dept'  class=\"btn btn-info\" role=\"button\">บันทึกบุคลากร</A></TD>
			<TD><A href='add_dept_edu.php?edit_dept_edu=$encode_id_dept'  class=\"btn btn-warning\" role=\"button\">แก้ไข</A></TD>
              ...
              <div class=\"box-tools pull-right\">
                ...
                <div class=\"btn-group\">
                  ...
                  <ul class=\"dropdown-menu\" role=\"menu\">
                    ...
                    <li><a href=\"select_position.php?manage_position=add\"><i class=\"fa fa-share\"></i> จัดการตำแหน่งาน</a></li>
                  </ul>
                </div>
