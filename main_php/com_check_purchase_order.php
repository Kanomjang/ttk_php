<?php
include("../lib_org.php");
$title_head_html = "ตรวจสอบใบสั่งซื้อ";
$position_active = '12';
...
insert_com_purchase_request($no_order,$title_cpr, $id_dept, $date_order,$date_delivery,$id_crmse,$unit_quantity,$unit_price,$comment_cpr);
...
##############################
function insert_com_purchase_request($no_order,$title_cpr, $id_dept, $date_order,$date_delivery,$id_crmse,$unit_quantity,$unit_price,$comment_cpr)//เพิ่มใบขอซื้อ
{
  global $host, $user, $passwd, $database, $id_emp_use;
  $connect = mysqli_connect($host, $user, $passwd, $database) or die("not connect database");
  $sql_query = "insert into com_purchase_request(no_order,title_cpr,id_dept,date_order,date_delivery,id_crmse,unit_quantity,unit_price,comment_cpr,id_use)
   values('$no_order','$title_cpr','$id_dept','$date_order','$date_delivery','$id_crmse','$unit_quantity','$unit_price','$comment_cpr','$id_emp_use')";
...
function com_purchase_request()
{
  ...
  $a = 0;
  $connect = mysqli_connect($host, $user, $passwd, $database) or die("not connect database");
  $sql_query = "SELECT id_purchasing_jobs,com_purchase_request.id_cpr,com_purchase_request.date_order,com_purchase_request.date_delivery,com_purchase_request.title_cpr,com_purchase_request.id_dept,com_purchase_request.id_use FROM com_purchase_request WHERE com_purchase_request.approve ='0' AND com_purchase_request.id_purchasing_jobs!=''";
  $shows = mysqli_query($connect, $sql_query);
  while ($row = mysqli_fetch_array($shows)) {
    ...
    $print[$a] = "
		<TR>
            ...
            <TD><A href='com_purchase_order_approval.php?com_purchase_request_report=$encode_id_cpr'  class=\"btn btn-info\" role=\"button\">อนุมัติใบขอซื้อ</A></TD>
		</TR>";
    $a++;
  }
  mysqli_close($connect);
...
function com_history_purchase_request()
{
  ...
  $a = 0;
  $connect = mysqli_connect($host, $user, $passwd, $database) or die("not connect database");
  $sql_query = "SELECT com_purchase_request.id_cpr,com_purchase_request.approve,com_purchase_request.date_order,com_purchase_request.date_delivery,com_purchase_request.title_cpr,com_purchase_request.id_dept,com_purchase_request.id_use FROM com_purchase_request WHERE com_purchase_request.id_purchasing_jobs!='' and approve!='0' and com_purchase_request.date_order between CURDATE()-30 and CURDATE() ORDER BY date_order DESC";
  $shows = mysqli_query($connect, $sql_query);
  while ($row = mysqli_fetch_array($shows)) {
    $id_cpr = $row["id_cpr"];
    $date_order = $row["date_order"];
  ...
  <div class=\"content-wrapper\">
    ...
    <section class=\"content-header\">
      ...
      <ol class=\"breadcrumb\">
        ...
        <li class=\"active\">ตรวจสอบใบสั่งซื้อ</li>
      </ol>
    </section>
    <!-- Info boxes -->
    <div class=\"row\">
        ...
        <div class=\"col-xs-12\">
          <div class=\"box\">
            <div class=\"box-header\">
              <h3 class=\"box-title\">ตรวจสอบใบสั่งซื้อ</h3>
              <div class=\"box-tools pull-right\">
                ...
                <div class=\"btn-group\">
                  ...
                  <li><a href=\"com_history_document_approval.php\"><i class=\"fa fa-share\"></i> ประวัติการตรวจสอบใบสั่งซื้อทั้งหมด</a></li>
                  </ul>
                </div>
              </div>
